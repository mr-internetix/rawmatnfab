...
router.get("", async (req, res) => {
	...
	if (currentCategoryData[0].categoryName) {
		const currentCategoryProducts = await Product.find({
			categoryName: currentCategoryData[0].categoryName,
		});
		res.render("category", {
			categoriesData: aboutsectionData,
			currentCategoryData: currentCategoryData,
			currentCategoryProducts: currentCategoryProducts,
			about: aboutsectionData,
		});
	} else {
		res.json({ message: "you have entered data in wrong category" });
	}
});
		...
		if (req.body.categoryName) {
			...
			if (currentCategory == null) {
				...
				if (req.files.length > 0) {
						...
						base64Image
							...
							.then((base64String) => {
								...
								imagekit
									...
									.then(async (response) => {
										data = await Category.findOneAndUpdate(
											{
												categoryName:
					...
					res.json({ message: "Category Uploaded SuccessFully" });
				} else {
