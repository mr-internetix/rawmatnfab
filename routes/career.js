...
router.post("", upload.single("resume_pdf"), async(req, res) => {
    if (req.file.mimetype === "application/pdf") {
        try {
            const resume_data = await careerInfo.find({ email: req.body.email_address });
            if (resume_data.length > 0) {
                res.json({ message: "your details already exists / Use another email if ypu want to post it  " });
            } else {
                const {
                    full_name,
                    email_address,
                ...
                const url = `${process.env.mainurl_with_port}/static/assets/resume/${req.file.filename}`;
                console.log(url);
